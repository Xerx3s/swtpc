...
def assess_pipeline(pipe: Pipeline, X_train, X_test, y_train, y_test):
    ...
    df_pred_test.rename(columns={0: "fpH"}, errors= "raise", inplace=True)
    ...
    actualvpredicted.to_csv(r"parts-testing\flocculation prediction\data\actualvpredicted_fpH.txt", sep=";", decimal=",")
...
# import dataset.
dataset = pd.read_excel(r"sustainable-drinking-water-treatment-plant\flocculation_analysis\data\calculations_fEC+fpH.xlsx", sheet_name="ml_fpH")

# prepare dataset. isolate then drop target column.
dataset.drop(columns=["d_pH"], inplace=True)
target = ["fpH"]
y = dataset[target]
